...
''
  ...
  export DISK_PART_LUKS="/dev/vda1"
  ...
  echo "LUKS: Creating ${swapSize} swap partition ..."
  ...
  echo "LUKS: Using rest of space for root partion..."
  lvcreate -l 100%FREE -n root "''${LUKS_VG_NAME}"

  export DISK_PART_ROOT="''${LUKS_VG_PATH}/root"
  mkfs.ext4 -L nixos "''${DISK_PART_ROOT}"
