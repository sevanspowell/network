...
#+BEGIN_SRC emacs-lisp
  ...
  (general-define-key
    :prefix  ","
    :states  'normal
    "k"  '(:ignore t :which-key "capture")
    "kk" 'org-capture
    "ki" 'org-capture-inbox
    "ka" 'org-agenda
...
#+BEGIN_SRC emacs-lisp
  ...
  (setq org-agenda-files (list "inbox.org" "calendar.org"))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  (setq org-todo-keywords
    (quote ((sequence "NEXT(n)" "TODO(t)" "IN PROGRESS(p)" "|" "DONE(d)")
...
(use-package org-gcal
  ...
  :after org
  :config
  (setq org-gcal-client-id (get-file-contents "/run/secrets/org_gcal_client_id")
        org-gcal-client-secret (get-file-contents "/run/secrets/org_gcal_client_secret")
        org-gcal-file-alist '(("mail@sevanspowell.net" .  "~/org/calendar.org")
                              ("georgialeebennett@gmail.com" . "~/org/calendar.org")
                              ))
  (defun my-org-gcal-sync ()
    (ignore-errors (org-gcal-sync)))

  (add-hook 'org-agenda-mode-hook 'my-org-gcal-sync)
  (add-hook 'org-capture-after-finalize-hook 'my-org-gcal-sync))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package ement
    ...
    (general-define-key
      ...
      "rr"  'ement-room-retro
      "rs"  'ement-room-sync
      "rc"  'ement-room-compose-message
      "rm"  'ement-room-send-message
      "rl"  'ement-room-list
    )
    ...
    (general-define-key
      ...
      "ec"  '((lambda () (interactive) (adrestia-connect)) :which-key "adrestia-chat")
      )
    ...
    (defun adrestia-connect ()
      (let* ((found (nth 0 (auth-source-search :host "localhost" :port "8008" :user "@sevanspowell:adrestia.iohkdev.io" :require '(:user :secret)))))
        (if found
            (let ((secret (plist-get found :secret))
                  (user (plist-get found :user)))
              (let ((secretpw (if (functionp secret) (funcall secret) secret)))
                (ement-connect :uri-prefix "http://localhost:8008" :user-id user :password secretpw)))
          nil
          )))
    :hook
    (ement-room-mode . (lambda () (set-window-fringes (selected-window) 0 0 nil)))
...
#+BEGIN_SRC emacs-lisp
  (use-package org-gcal
    ...
    (setq org-gcal-client-id ""
          ...
          org-gcal-fetch-file-alist '(("samuel.evans-powell@iohk.io" .  "~/schedule.org")))
    )
#+END_SRC
...
(use-package obsidian
  ...
  :demand t
  :config
  (obsidian-specify-path "~/docs/obsidian")
